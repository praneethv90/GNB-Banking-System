    ...
    <div class="row justify-content-center align-items-center" style="min-height: 100vh;">
        <div class="col-md-6">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="Functions\checqueWithdraw.php" method="post" onsubmit="return validateWithdrawal()">
                        <div class="form-group mb-3">
                            <input type="text" placeholder="Amount" class="form-control" name="amount">
                            <input type="hidden" value="<?php echo $accountNo; ?>" class="form-control" name="accountNo">
                        </div>
                        <div class="form-group mb-3">
                            <div class="form-group mb-3">
                            ...
                            <div class="form-group mb-3">
                                <select name="chequeNo" class="form-control" required>
                                    <option value="" disabled selected>Please select your cheque</option>
                                    <?php
                                    include 'Functions/dbcon.php';
                                    $stmt2 = $con->prepare("CALL getavailableCheques(?)");
                                    $stmt2->bind_param("i", $accountNo);
                                    $stmt2->execute();
                                    $result2 = $stmt2->get_result();

                                    while ($row2 = $result2->fetch_assoc()) {
                                        $chequeNo = $row2['chequeNo'];
                                        echo '<option value="' . $chequeNo . '">' . $chequeNo . '</option>';
                                    }
                                    ?>
                                </select>
                            </div>
                            <div class="form-group mb-3">
                                <input type="textarea" placeholder="Consignee Name" class="form-control" name="consignee" required>
                            </div>
                            <div class="form-group mb-3">
                                <input type="textarea" placeholder="Consignee NIC" class="form-control" name="consigneeID" required>
                            </div>

                            <div id="warningMessage" class="text-danger justify-content-center align-items-center">
                            </div>
                            <div class="text-center">
                                <button class="btn btn-info">Withdraw Cheque</button>
                            </div>
                        </div>
